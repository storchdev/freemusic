...
fn build(sysroot: Option<&str>) -> io::Result<()> {
    ...
    if target != host {
        ...
        // Tune the compiler for the host architecture. Skipped under MSVC: `cl.exe` has no
        // `-march`/`-mtune` equivalent at all (that's GCC/Clang flag syntax), and passing it
        // fails configure's own C-compiler smoke test outright with
        // "cl : Command line error D8043 : unknown option '-mtune=native'" — surfaced upstream
        // as the more generic-sounding "cl.exe is unable to create an executable file". Upstream
        // ffmpeg-sys-next (post-8.1.0, unreleased as of this vendoring) added an env-var escape
        // hatch (`FFMPEG_MARCH`/`FFMPEG_MTUNE`) for this, but that doesn't help on Windows either
        // way: Win32's `SetEnvironmentVariable` (which PowerShell/.NET use) treats an empty-string
        // value as "delete the variable", so `FFMPEG_MARCH=""` can't actually be represented in a
        // child process's environment block, and the crate would silently fall back to the
        // hardcoded default anyway. Special-casing MSVC here instead of relying on an env var
        // sidesteps that Windows OS-level limitation entirely.
