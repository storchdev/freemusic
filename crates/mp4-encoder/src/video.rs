...
use crate::{ff, SRC_STREAM_PIX_FMT, STREAM_PIX_FMT};
...
impl VideoOutputStream {
    pub fn new(
        ...
        // Prefer libx264 by name over `output_format.video_codec()` (`avcodec_find_encoder` by
        // ID) — see `ff::Codec::find_by_name` for why that's not deterministic enough for us.
        let codec =
            ff::Codec::find_by_name(c"libx264").unwrap_or_else(|| output_format.video_codec());
        ...
        unsafe {
            ...
            // One intra frame per second of output, rather than upstream's hardcoded `12`
            // (tuned for its own hardcoded 60fps) — scales with whatever `fps` the caller asked
            // for.
            (*codec_ctx).gop_size = fps.max(1) as i32;
