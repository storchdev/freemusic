...
pub fn new_audio_streams(
    ...
    assert_ne!(
        output_format.audio_codec_id(),
        AVCodecID::AV_CODEC_ID_NONE,
        "The selected output container does not support audio encoding"
    );

    // Prefer AAC by name — same rationale as the video side's `libx264` preference in
    // `video.rs`, kept explicit rather than trusting `output_format.audio_codec()`.
    let codec = ff::Codec::find_by_name(c"aac").unwrap_or_else(|| output_format.audio_codec());
