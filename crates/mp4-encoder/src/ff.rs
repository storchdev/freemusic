...
impl Codec {
    /// Looks up an encoder by its registered name (e.g. `c"libx264"`), rather than by codec ID —
    /// `avcodec_find_encoder(id)` picks whichever encoder for that ID happens to be registered
    /// first, which is nondeterministic when more than one is available (e.g. a hardware
    /// encoder ahead of libx264). Added on top of upstream's `ff.rs` specifically so callers can
    /// pin a concrete encoder instead.
    pub fn find_by_name(name: &std::ffi::CStr) -> Option<Self> {
        let codec = unsafe { ffmpeg::avcodec_find_encoder_by_name(name.as_ptr()) };
        (!codec.is_null()).then_some(Codec(codec))
    }
