...
// with one real change: the hit line ("keyboard_y") is no longer hardcoded to 80% of
// `view_uniform.size.y` — it reads `view_uniform.barrier_fraction` instead, so the barrier can
// sit anywhere without the viewport-remapping hack `midi_overlay.rs` used to need (see
// CLAUDE.md's "Vendor note pipeline" section for why that hack existed and why owning this
// shader lets it be deleted). `velocity`/`track_index` are carried as instance attributes for a
// future property-binding phase but are not yet read here.
...
struct NoteInstance {
    ...
    @location(5) velocity: f32,
    @location(6) track_index: f32,
}
